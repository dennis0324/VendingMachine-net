...
CREATE DEFINER=`admin`@`%` PROCEDURE `DELIMIT_VALUES`(
...
CREATE DEFINER=`admin`@`%` PROCEDURE `FUNCTION_DELIMIT2_GET_INDEX`(
    IN valData Text,
...
CREATE DEFINER=`admin`@`%` PROCEDURE `FUNCTION_SPLIT_DELIMITER`(
    IN valData TEXT,
...
CREATE DEFINER=`admin`@`%` PROCEDURE `FUNCTION_SPLIT_GET_INDEX`(
    IN valData TEXT,
    IN idx INT,
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_HIST_ADD`(
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_HIST_GET`(
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_INIT`(
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_MONEY`(
    IN cmd varchar(5),
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_MONEY_INSERT`(
  IN vendingId varchar(5),
  IN valdata TEXT
)
BEGIN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_PRODUCT`(
    IN cmd varchar(5),
    IN inputvendingId varchar(5),
    IN time TIMESTAMP,
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_REMOVE`(
    IN machineID varchar(5)
)
BEGIN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `MACHINE_REMOVE_ALL`()
BEGIN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `USER_LOGININFO_GET`(
...
CREATE DEFINER=`admin`@`%` PROCEDURE `USER_PASSWORD_CHANGE`(
...
GRANT EXECUTE ON PROCEDURE MACHINE_LOGININFO_GET TO 'vending'@'%';
GRANT EXECUTE ON PROCEDURE MACHINE_PASSWORD_CHANGE TO 'vending'@'%';